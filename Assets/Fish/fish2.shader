		...
		pass {   
			...
			*/
			...
			float4 frag (v2f i) : COLOR {
				...
				uv_uv1 = uv_uv1+fixed2(-0.2174652,-0.001259923)*(_Time.y);    
				...
				uv_uv1 = uv_uv1+center_uv1;    
				float2 uv_uv1orgin = uv_uv1;
				uv_uv1 = float2(uv_uv1.x >0 ?fmod(uv_uv1.x,1+0) : (1+0) - fmod(abs(uv_uv1.x),1+0), uv_uv1.y >0 ?fmod(uv_uv1.y,1+0): (1+0) - fmod(abs(uv_uv1.y),1+0));
				bool discard_uv1 = false;
				if(uv_uv1.x>1 || uv_uv1.y>1)
					discard_uv1 = true;
				float4 rect_uv1 =  float4(1,1,1,1);
				float4 color_uv1 = tex2D(_wave2,uv_uv1);    
				if(discard_uv1 == true) color_uv1 = float4(0,0,0,0);
				uv_uv1 = -(color_uv1.r*fixed2(-0.05965316,-0.04125178) + color_uv1.g*fixed2(-0.005721271,0.0817616) + color_uv1.b*fixed2(0.06946641,-0.04390872) +  color_uv1.a*fixed2(0,0));    
