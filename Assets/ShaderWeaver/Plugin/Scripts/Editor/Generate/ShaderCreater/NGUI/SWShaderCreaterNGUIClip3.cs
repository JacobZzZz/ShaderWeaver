...
namespace ShaderWeaver
{
	...
	/// Create Shader
	/// </summary>
	public class SWShaderCreaterNGUIClip3:SWShaderCreaterNGUIClipSoftBase{
		public SWShaderCreaterNGUIClip3(SWWindowMain _window):base(_window)
		{
			clipCount = 3;
			InitName ();
		}
		protected override void Struct_v2f ()
		{
			...
			StringAddV2d ("float2", "worldPos2", "COLOR4");
		}
		protected override void Vert ()
		{
			...
			StringAddLine ("\t\t\t\tOUT.worldPos2 = NGUI_Rotate(IN.vertex.xy, _ClipArgs2.zw) * _ClipRange2.zw + _ClipRange2.xy;");
		}
		public override void ProcessExtra (SWNodeBase root)
		{
			...
			StringAddLine ("\t\t\t\t// Third clip region");
			StringAddLine ("\t\t\t\tfactor = (float2(1.0, 1.0) - abs(i.worldPos2)) * _ClipArgs2.xy;");
			StringAddLine ("\t\t\t\tf = min(f, min(factor.x, factor.y));");
			//step 2:Mul aplha
