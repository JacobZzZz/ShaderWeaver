		...
		pass {   
			...
			*/
			...
			float4 frag (v2f i) : COLOR {
				...
				uv_alpha = float2(uv_alpha.x >0 ?fmod(uv_alpha.x,1+0) : (1+0) - fmod(abs(uv_alpha.x),1+0), uv_alpha.y >0 ?fmod(uv_alpha.y,1+0): (1+0) - fmod(abs(uv_alpha.y),1+0));
				...
				uv_dummy7 = float2(uv_dummy7.x >0 ?fmod(uv_dummy7.x,1+0) : (1+0) - fmod(abs(uv_dummy7.x),1+0), uv_dummy7.y >0 ?fmod(uv_dummy7.y,1+0): (1+0) - fmod(abs(uv_dummy7.y),1+0));
